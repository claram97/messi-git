use std::{io::{Read, self, Write}, path::{Path, PathBuf}, fs::{File, self}};

use crate::{utils, branch, commit};

///Pre: recibe el path al git directory (no al base directory)
///Post:
...
pub fn create_new_branch(git_dir: &str, branch_name : &str,  output: &mut impl Write) -> io::Result<()> {
    let heads_dir = (&git_dir).to_string() + "/refs/heads";
    let entries = fs::read_dir(heads_dir)?;
    if entries.count() == 0 {
        let buffer = format!("fatal: Please commit something to create a branch\n");
...
// if path.exists() {
//     let buffer = format!("fatal: A branch named '{}' already exists",branch_name);
// }
...
mod tests {
    ...
    fn test_list_branches() -> Result<(), io::Error> {
        ...
        create_if_not_exists("tests/test_list_branches/.mgit/refs/heads/current_branch", false)?;
    ...
    fn test_create_new_branch_already_exists() -> Result<(), io::Error> {
        ...
        create_if_not_exists("tests/test_list_branches_3/.mgit/refs/heads/current_branch", false)?;
        let mut output : Vec<u8> = vec![];
        create_new_branch("tests/test_list_branches_3/.mgit", "current_branch", &mut output)?;
        assert!(!output.is_empty());
    ...
    fn test_create_new_branch() -> Result<(), io::Error> {
        ...
        create_if_not_exists("tests/test_list_branches_4/.mgit/refs/heads/branch", false)?;

        let mut output : Vec<u8> = vec![];
        create_new_branch("tests/test_list_branches_4/.mgit", "my_branch", &mut output)?;
        assert!(output.is_empty());
        //Descomentar esta línea cuando funcione

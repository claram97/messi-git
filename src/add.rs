    ...
    if fs::metadata(file_name)?.is_dir() {
        for entry in fs::read_dir(file_name)? {
            ...
            if file_path.is_file() {
                let string_file_path = &file_path.to_string_lossy().to_string();
                index.add_path(string_file_path)?;
            }
        }
    } else {
...
pub fn add(
    ...
    if let Some(params) = options {
        if params.len() == 1 && params[0] == "." {
            ...
            if !current_dir_str.eq(".mgit") && !current_dir_str.eq(".git") {
        ...
        if !path.starts_with(".git") && !path.starts_with(".mgit") {
            let mut index = Index::load(index_path, git_dir_path, gitignore_path)?;

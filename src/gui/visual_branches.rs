...
fn define_branch_colors(
    ...
    for branch in branches {
        let x_pos = branches_pos.get(branch).unwrap_or(&20).clone();
...
fn draw_branch_names(
    ...
    for branch in branches {
        ...
        let x_pos = x_positions.get(&name).unwrap_or(&20).clone();
        y_pos = y_pos + 20;
        drawing_area.connect_draw(move |_, context| {
...
fn draw_commit_node_w_message_side(
    ...
    drawing_area.connect_draw(move |_, context| {
        ...
        context.arc(x as f64, y as f64, COMMIT_RADIUS, 0.0, 2.0 * 3.14);
        context.fill();
        context.set_source_rgb(0.0, 0.0, 0.0);
        context.move_to(message_x as f64 + 5.0, message_y as f64 - 5.0 );
        context.show_text(&hash[..7]);
        context.move_to(message_x as f64 + 5.0, message_y as f64 + 10.0);
        context.show_text(&message);
        context.set_source_rgb(color.0, color.1, color.2);
        context.move_to(message_x as f64, message_y as f64 - 15.0);
        context.line_to(message_x as f64, message_y as f64 + 15.0);
        context.stroke();

        context.set_source_rgba(color.0, color.1, color.2, 0.25);
        context.move_to(message_x as f64, message_y as f64);
...
fn draw_commit_nodes_messages(
    ...
    for commit in commits {
        ...
        let branch_x = branches_pos.get(&branch).unwrap_or(&20).clone();
        let color = branch_colors.get(&branch_x).unwrap_or(&(0.0, 1.0, 0.22)).clone();
        let hash = commit.hash.clone();
        let message = commit.message.clone();
        let message_x = BRANCHES_DISTANCE * branches_len;
        let message_y = y;
        draw_commit_node_w_message_side(drawing_area, branch_x, y, message_x, message_y, color, hash.clone(), message.clone());

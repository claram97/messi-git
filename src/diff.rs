...
fn read_file_lines(path: &str) -> Result<Vec<String>, String> {
    match File::open(path) {
        Ok(file) => {
            let lines_vector: Result<Vec<String>, std::io::Error> = BufReader::new(file).lines().collect();
            match lines_vector {
                Ok(lines) => Ok(lines),
    ...
    for i in 0..a.len() {
        for j in 0..b.len() {
            if a[i] == b[j] {
                ...
                matrix[i+1][j+1] = std::cmp::max(matrix[i+1][j], matrix[i][j+1]);
            }
        }
    }
...
pub fn return_diff(path_a: &str, path_b: &str) -> Result<Vec<String>, String> {
    ...
    Ok(diff_to_vec(&archivo_a, &archivo_b, archivo_a.len(), archivo_b.len()))
}
...
pub fn return_object_diff_string(hash_a: &str, hash_b: &str, git_dir: &str) -> Result<String, String> {
    ...
    let object_a_vec = object_a.split('\n').map(|s| s.to_string()).collect::<Vec<String>>();
    let object_b_vec = object_b.split('\n').map(|s| s.to_string()).collect::<Vec<String>>();

    let mut output: Vec<String> = Vec::new();
    for line in diff_to_vec(&object_a_vec, &object_b_vec, object_a_vec.len(), object_b_vec.len()) {
        output.push(line);
    }
